...
export default function Ranks() {
  ...
  const sortedUsers = [...users].sort((a, b) => (b.ranking ?? b.level) - (a.ranking ?? a.level))
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold text-center mb-6">🏆 Rankings</h1>
      ...
      {activeTab === "users" && (
        <div className="bg-stone-900 rounded-xl p-4 space-y-3 border border-stone-700">
          {sortedUsers.map((user, i) => (
            <div
              key={user._id}
              className="flex justify-between items-center p-3 rounded-lg bg-stone-800"
            >
              ...
              <div className="flex items-center gap-2 text-yellow-300">
                <FaTrophy />
                <span className="font-medium">{user.ranking ?? user.level}</span>
              </div>
            </div>
          ))}
        </div>
